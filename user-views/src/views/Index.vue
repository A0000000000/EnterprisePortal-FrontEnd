<template>
  <div class="index">
    <div id="enterpriseName">
      <img src="../assets/images/milogo.png" alt="小米" />
      <span>小米公司</span>
    </div>
    <p>{{ tipsData }}</p>
    <div id="content">
...
export default {
  ...
  mounted() {
    let timer = () => {
      if (this.index < this.tips.length) {
        this.index++;
        setTimeout(timer, 300 + Math.random() * 100);
      }
    };
    timer();
  },
  computed: {
    tipsData() {
      return (
        this.tips.substring(0, this.index) +
        (this.index === this.tips.length ? "" : "__")
      );
    },
  },
};
